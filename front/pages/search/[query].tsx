...
export const getServerSideProps: GetServerSideProps = async ({ params }) => {
  ...
  let response = await fetch(
    "https://globalmarkets.herokuapp.com/products"
  ).then((res) => res.json());
  let products = response.filter((i: any) => {
    ...
    if (i.title.toLowerCase().includes(find)) return i;
      ...
      !!i.caterogiras &&
      i.caterogiras.length > 0 &&
      i.caterogiras[0].toLowerCase().includes(find)
    )
      return i;
  });
  const foundProducts = products.length > 0;
  if (!foundProducts) {
    let data = await fetch("https://globalmarkets.herokuapp.com/products").then(
      (res) => res.json()
    );
    return {
      props: {
